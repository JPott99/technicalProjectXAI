...
for k in range(loops0):
        ...
        for j in range(subloops):
            ...
            while continueLooping == True:
                ...
                for i in range(len(agentArray)):
                    agentArray[i] = agentAction(agentArray[i], environmentReliability, agentArray, theTruth,randP1[i], 0.35,randP2[i])
    ...
    # for i in agentArray:
    #     print(i)#
plt.imshow(timeArray, cmap='YlOrRd', origin='lower', extent=[0,0.35,0,1],aspect='auto')
plt.colorbar()
plt.ylabel(r"Flexibility Variance")
plt.xlabel(r"Environment Learning Rate Variance")
plt.title(r"Heatmap showing mixed learning rate and $\alpha$ performance.")
plt.savefig(folderName+"mixedAgentFlexEleHeatmapA"+str(int(aRel*100))+"E"+str(int(eRel*100))+".png")
#plt.show()
